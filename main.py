...
def verify_pw(pw: str, stored_hash: str) -> bool:
    """Thử TẤT CẢ format hash có thể để backward-compat."""
    checks = [
        hashlib.pbkdf2_hmac("sha256", pw.encode(), b"caro309_fixed_salt_v1", 100_000).hex(),
        hmac.new(SECRET.encode(), pw.encode(), hashlib.sha256).hexdigest(),
        hashlib.sha256((SECRET + pw).encode()).hexdigest(),
        hashlib.sha256(pw.encode()).hexdigest(),
        hmac.new(pw.encode(), SECRET.encode(), hashlib.sha256).hexdigest(),
...
def do_move(body: MoveIn):
    ...
    with _ai_lock:
        ...
        if not has_pieces:
            raise HTTPException(400, "Board trống")
...
class Rooms:
    ...
    def create(self, size=20, creator="") -> str:
    ...
    def info(self, rid: str) -> dict:
        ...
        return {
            "room_id": rid, "status": r.get("status", "?"),
            "size": r.get("size", 20),
    ...
    async def remove(self, ws: WebSocket, rid: str):
        ...
        if role and room.get("winner"):
            left_msg = {"type": "player_left", "username": username, "winner": room["winner"]}
            if elo_result:
    ...
    try:
        ...
        db.execute("INSERT INTO games VALUES (?,?,?,?,?,?,?)",
                   (str(uuid.uuid4()), xn, on, winner,
                    room.get("moves", 0), room.get("size", 20), int(time.time())))
        db.commit(); db.close()
...
def create_room(body: RoomIn, user=Depends(get_user)):
    rid = rooms.create(max(15, min(30, body.size)), user["name"])
    return {"room_id": rid}
...
async def ws_endpoint(ws: WebSocket, rid: str, token: str = ""):
    ...
    await ws.send_json({
        "type": "joined", "role": role, "username": username,
        "room": rooms.info(rid), "board": room["board"],
